...
const mainMenuDropdown = function() {
  const mainMenuItems = document.querySelectorAll('.main__menu--items');
  
  if (mainMenuItems) {
    mainMenuItems.forEach(function(item) {
      item.addEventListener('click', function(e) {
        const hasSubmenu = this.querySelector('.sub__menu');
        if (hasSubmenu) {
          e.preventDefault();
          this.classList.toggle('active');
          const subMenu = this.querySelector('.sub__menu');
    ...
    document.addEventListener('click', function(e) {
      if (!e.target.closest('.main__menu--items')) {
        mainMenuItems.forEach(function(item) {
  ...
  countryList.forEach((country) => {
    country?.addEventListener("click", (event) => {
      ...
      countryThumbList.forEach((countryThumb) => {
        ...
        if (
          countryThumb.dataset.locationName === country.dataset.locationName
        ) {
